...
export default class CesiumScene {
  ...
  async addModelToCesium() {
    const resource = await Cesium.IonResource.fromAssetId(2801507)
    this.viewer.entities.add({
      model: { uri: resource },
      position: Cesium.Cartesian3.fromDegrees(-121.879, 37.3418, 0),
    })
  }
  ...
  moveBabylonCamera() {
    const fov = Cesium.Math.toDegrees((this.viewer.camera.frustum as Cesium.PerspectiveFrustum).fovy || 1)
    this.camera.fov = fov / 180 * Math.PI

    const civm = this.viewer.camera.inverseViewMatrix.clone()
    ...
    const camera_pos = this.cart2vec(new Cesium.Cartesian3(transform.x, transform.y, transform.z))
  ...
  cart2vec(cart: Cesium.Cartesian3): Vector3 {
